...
export const calloutText = {
  ...
  context: "This demo highlights two of ClearBrief's many features: source discovery ('Add Fact Cite') and citation verification ('Cite Check'). ClearBrief also generates hyperlinked timelines, assembles exhibits, creates tables of authorities, builds cross-examination outlines, and more — we'll cover some of those features in later classes. The source discovery workflow lets you select any sentence and instantly surface the best supporting passages from your uploaded record. The verification workflow analyzes every citation to confirm the brief accurately represents the source — different from hallucination-checking (which Westlaw and Lexis brief analyzers handle). Here, every citation is to a real document; the question is whether the brief accurately represents what the source says."
}
